...
//DNS packet
size_t make_packet(size_t loc, unsigned char *buf, struct header *header, struct query q1, struct record **answers, struct record **auths, struct record **adds) {
	//Transaction ID
	...
	loc += sizeof(add_rr);
		...
		unsigned short type = q1.type;
		unsigned short class = q1.class;
